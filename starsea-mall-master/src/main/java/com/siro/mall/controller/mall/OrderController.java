...
//        Set<Long> goodsIdSet=new HashSet<>();
        Map<Long,Integer> goodsIdMap=new HashMap<>();
        for(ShoppingCartItemVO item: myShoppingCartItems){
            goodsIdMap.put(item.getGoodsId(),item.getGoodsCount());
        }
        //保存订单并返回订单号
        String saveOrderResult = orderService.saveOrder(user, myShoppingCartItems);
        //减库存
//        goodsService.reductionStockNum(goodsIdMap);
        //跳转到订单详情页
    ...
    public Result paySuccess(@RequestParam("orderNo") String orderNo,
        ...
        if(payType==3){
            ...
            UserVO userVO = userService.reductionUserBalance(user, httpSession);
            if(userVO==null){
    ...
    @ApiOperation(value = "查看我的订单列表，带条件")
    @GetMapping("/conditionsOrders")
    public String orderList(@RequestParam Map<String, Object> params,
        ...
        //封装我的订单数据
        PageQueryUtil pageUtil = new PageQueryUtil(params);
        request.setAttribute("orderPageResult", orderService.getMyOrdersConditions(pageUtil));
        request.setAttribute("path", "orders");
        return "mall/my-orders";
    }
    ...
    @PostMapping("/orders/mark")
    @ResponseBody
    public Result markOrder(GoodsMark goodsMark){
        goodsMark.setReviewTime(new Date());
        goodsMarkService.saveGoodsMark(goodsMark);
        return ResultGenerator.genSuccessResult();
    }
